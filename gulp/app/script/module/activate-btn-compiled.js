...
(function (root, factory) {
...
})(undefined, function (root, exports) {

    var CHECK = 'data-activate-check',
        // 要检查的input需要添加的属性
    SUBMIT = 'data-activate-submit',
        // 关联按钮要添加的属性
    DISABLED = 'disabled'; // 按钮禁用添加的class

    var btnLight = function btnLight(obj) {
    ...
    btnLight.prototype = {
        ...
        _iteration: function _iteration(inputArr) {
            var self = this,
                inputArr = self._getWants(self.contain, CHECK, 'input');
        ...
        _eval: function _eval(val) {
            return new Function("return " + val)();
        },

        _getWants: function _getWants(oParent, oWants, tag) {
            var i,
        ...
        _addClass: function _addClass(obj, cls) {
            ...
            if (typeof cls == 'string' && targetObj.nodeType === 1) {
                if (!targetObj.className) {
                    ...
                    a.sort();
                    for (var i = a.length - 1; i > 0; --i) if (a[i] == a[i - 1]) a.splice(i, 1);
                    targetObj.className = a.join(' ');
                }
            }
        },
        _removeClass: function _removeClass(obj, cls) {
            var targetObj = this.isErrorOnParent ? obj.parentElement : obj;
...
//# sourceMappingURL=activate-btn-compiled.js.map